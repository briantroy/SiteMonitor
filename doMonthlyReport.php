...
/*
 * This script will generate a monthly report for the previous
 * day.
...
// Last Month's Date
$lastMonth = strtotime("-1 month"); // time() - $intOffset;
$strDate = date('r', $lastMonth);
$dateParts = explode(" ", $strDate);
$strDate = $dateParts[2]." ".$dateParts[3];
...
$msg = "<html><head></head><body><h1>Site Monitor Report for: ".date("F, Y", $lastMonth)."</h1>";
...
sendEmails($msgText, $msg, "Monthly SiteMonitor Report", $aryAlertUsers);
...
function sendEmails($tMsg, $hMsg, $subj, $aryAlertTo) {
    ...
    foreach($aryAlertTo['user'] as $user) {
        foreach($user['alertmethods'] as $method) {
            if($method['type'] == "EMAIL" && $method['active'] && $method['monthly']) {
