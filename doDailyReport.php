...
/*
 * This script will generate a daily report for the previous 
 * day.
...
// Yesterday's date
$yestDate = strtotime("-1 day");
$strDate = date('r', $yestDate);
$dateParts = explode(" ", $strDate);
$strDate = $dateParts[1]." ".$dateParts[2]." ".$dateParts[3];
...
$msg = "<html><head></head><body><h1>Site Monitor Report for: ".date("l F jS Y", $yestDate)."</h1>";
...
sendEmails($msgText, $msg, "Daily SiteMonitor Report", $aryAlertUsers);
...
function sendEmails($tMsg, $hMsg, $subj, $aryAlertTo) {
    ...
    foreach($aryAlertTo['user'] as $user) {
        foreach($user['alertmethods'] as $method) {
            if($method['type'] == "EMAIL" && $method['active'] && $method['daily']) {
